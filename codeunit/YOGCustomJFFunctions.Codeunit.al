codeunit 14228880 YOGCustomJFFunctions
{
    ...
    var
        grecTempUDCalcValues: Record Buffer temporary;
        gcduUDCalculations: Codeunit YOGCustomJFFunctions;
        ...
        OrderSignature: Record "Sales Order Signature";
        SignatureSetup: Record "Signature Capture Setup";
        Topaz: Boolean;
        CLine: Text[100];
        Return: Integer;
        Param: Text[100];
    ...
    begin
        ...
        if pblnShowError and not lrecItemUOM."Allow Variable Qty. Per" then begin

            lrecItemUOM.FieldError("Allow Variable Qty. Per");
        end;

        exit(lrecItemUOM."Allow Variable Qty. Per");
    end;
    ...
    begin
        ...
        lrecItemUOM.SetFilter(lrecItemUOM."Qty. per Base UOM", '>%1', lrecItemUOM."Qty. per Base UOM");
    ...
    begin
        ...
        if (precSalesHeaderVAR."Authorized Amount" < precSalesHeaderVAR."Amount Including VAT") then begin
            T36CalcCredit(precSalesHeaderVAR, Outstanding, Limit);
            if (Outstanding[2] > 0) or
              (PaymentMethod.Cash and (Outstanding[1] > Limit)) or
              ((not PaymentMethod.Cash) and ((Outstanding[1] + precSalesHeaderVAR."Amount Including VAT" -
                precSalesHeaderVAR."Cash Applied (Current)") > Limit))
            then begin
                if PromptForApproval then begin
                    ...
                    if T36AuthorizeOrder(precSalesHeaderVAR, Outstanding, Limit) = true then begin
                        if precSalesHeaderVAR."Authorized Amount" >= precSalesHeaderVAR."Amount Including VAT" then begin
                            ...
                            Error('Order is authorized for %1.', precSalesHeaderVAR."Authorized Amount");
                        end;
                    end else begin
                        exit(false);
                    end;
    ...
    begin
        ...
        DueDate := TODAY - Customer."Credit Grace Period (Days)";
        ...
        IF CustLedger.FIND('-') THEN
            REPEAT
                ...
                IF CustLedger."Due Date" < DueDate THEN BEGIN
                    IF (CustLedger."Remaining Amount" < 0) OR (CustLedger."Remaining Amount" >= SalesSetup."C&C Minimum Overdue Invoice") THEN
    ...
    var
        ApproveOrder: Page "Approve Order";
        User: Code[20];
        Authorized: Decimal;
        lboolReturn: Boolean;
    ...
    begin
        ...
        //<<EN1.00
        precSalesHeaderVAR.CalcFields("Amount Including VAT");
        if precSalesHeaderVAR."Authorized Amount" <> 0 then
            Authorized := precSalesHeaderVAR."Authorized Amount"
        else
            Authorized := precSalesHeaderVAR."Amount Including VAT";
        ApproveOrder.SetVariables(Outstanding, Limit, precSalesHeaderVAR."Amount Including VAT", Authorized, precSalesHeaderVAR."Cash Applied (Current)");
        Commit;
        if ApproveOrder.RunModal = ACTION::OK then begin
            ApproveOrder.GetVariables(User, Authorized);
            if User <> '' then begin
                precSalesHeaderVAR."Authorized User" := User;
                precSalesHeaderVAR."Authorized Amount" := Authorized;
    ...
    begin
        ...
        //  EXIT(TRUE);
        IF precSalesLineVAR."Pricing Method" = precSalesLineVAR."Pricing Method"::"Volume Discount" THEN EXIT(TRUE);
        IF (precSalesLineVAR."Authorized Unit Price" >= 0) AND (precSalesLineVAR."Authorized Unit Price" <= Price) THEN EXIT(TRUE);
    ...
    begin
        ...
        T37GetItem(precSalesLineVAR);
        EXIT((grecItem."Unit Price" * precSalesLineVAR."Qty. per Unit of Measure") - (grecItem."Minimum Price Delta" * precSalesLineVAR."Qty. per Unit of Measure"));
    ...
    begin
        ...
        T37GetItem(precSalesLineVAR);
        EXIT(grecItem."Estimated Average Cost" * precSalesLineVAR."Qty. per Unit of Measure");

    end;

    procedure T37AuthorizePrice(VAR precSalesLineVAR: Record "Sales Line")
    var
        PriceAuthorization: Page "Price Authorization";
        MinPrice: Decimal;
        AuthUser: Code[20];
        PriceOK: Boolean;
    begin
        ...
        IF T37PriceCheck(precSalesLineVAR) THEN BEGIN
            precSalesLineVAR."To be Authorized" := FALSE;
        ...
        END;
        precSalesLineVAR."Authorized Unit Price" := precSalesLineVAR."Unit Price";// - T37BottleDeposit(precSalesLineVAR);TBR
        precSalesLineVAR."To be Authorized" := FALSE;
        precSalesLineVAR.MODIFY;
        IF PriceOK THEN
            precSalesLineVAR."Authorized Price below Cost" := USERID //lrecItemAuth."User ID"TBR
        ELSE
