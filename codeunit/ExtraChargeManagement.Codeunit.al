codeunit 14229100 "Extra Charge Management"
{
    trigger OnRun()
    begin
    end;
    ...
    var
        ...
        VendorBuffer: Record "Extra Charge Vendor Buffer";
        ...
        DropShipPostingBuffer: Record "Extra Charge Posting Buffer";
        ExtraCharge: Boolean;
    ...
    var
        ...
        PstdDocExtraCharge: Record "Posted Document Extra Charges";
        CurrExchRate: Record "Currency Exchange Rate";
        Currency2: Record Currency;
        SignFactor: Integer;
        PstdDocExtraCharge1: Record "Posted Document Extra Charges";
    ...
    procedure AdjustItemJnlLine(var ItemJnlLine: Record "Item Journal Line"; var PostingBuffer: Record "Extra Charge Posting Buffer"; var ExtraChargeQuantity: Decimal)
    var
    ...
    begin
        ...
        if ItemJnlLine."Entry Type" = ItemJnlLine."Entry Type"::Purchase then begin
            ...
            if ItemJnlLine."Invoiced Quantity" <> 0 then
                Quantity := ItemJnlLine.GetCostingQty(ItemJnlLine.FieldNo("Invoiced Quantity"))
            else
                Quantity := ItemJnlLine.GetCostingQty(ItemJnlLine.FieldNo(Quantity));
        ...
        if DocExtraCharge.Find('-') then
            repeat
                ...
                if TableNo in [120, 121] then begin
                    ...
                    PurchInvLine.SetRange("Extra Charge Code", DocExtraCharge."Extra Charge Code");
                    PurchInvLine.SetFilter("Purch. Order for Extra Charge", '%1', DocNo + '*');
                ...
                if TableNo in [122, 123] then begin
                    ...
                    //<<EN 081019 Rpatel
                    PurchInvLine.Reset;
                    PurchInvLine.SetCurrentKey(Type, "No.", "Purch. Order for Extra Charge", "Extra Charge Code");
                    PurchInvLine.SetRange("Extra Charge Code", DocExtraCharge."Extra Charge Code");
                    PurchInvLine.SetFilter("Purch. Order for Extra Charge", '%1', DocNo + '*');
            ...
            repeat
                ...
                if ChargeLCY <> 0 then begin
                    ...
                    if TableNo in [120, 121] then begin
                        ...
                        PurchInvLine.Reset;
                        PurchInvLine.SetCurrentKey(Type, "No.", "Purch. Order for Extra Charge", "Extra Charge Code");
                        PurchInvLine.SetRange("Extra Charge Code", ECPostingBuffer."Extra Charge Code");
                        PurchInvLine.SetFilter("Purch. Order for Extra Charge", '%1', DocNo + '*');
                        if PurchInvLine.FindFirst then
                            PstdDocExtraCharge.Status := PstdDocExtraCharge.Status::Closed;
                        //>>EN 081019
    ...
    var
        SourcePstdDocExtraCharge: Record "Posted Document Extra Charges";
        TargetPstdDocExtraCharge: Record "Posted Document Extra Charges";
    begin
    ...
    var
        PstdDocExtraCharge: Record "Posted Document Extra Charges";
        PstdDocLineExtraCharge: Record "Posted Document Extra Charges";
    begin
    ...
    procedure SetBufferForItemPosting(var PostingBuffer: Record "Extra Charge Posting Buffer"; Qty: Decimal; OrderNo: Code[20])
    begin
    ...
    begin
        ...
        if not PurchaseLineTotals.Get(DocType, DocNo) then begin
            ...
            if PurchaseLine.Find('-') then
                repeat
                    ...
                    PurchaseLineTotals."Pallet Count" += PurchaseLine."Pallet Count";
        ...
        LineTotals[DocExtraCharge."Allocation Method"::Pallet] += PurchaseLineTotals."Pallet Count";
    ...
    var
        PstdDocExtraCharge: Record "Posted Document Extra Charges";
        PstdDocExtraCharge2: Record "Posted Document Extra Charges";
        PurchRcptLine: Record "Purch. Rcpt. Line";
    ...
    begin
            ...
            REPEAT
                ...
                IF VendorBuffer.FIND('-') THEN
                    REPEAT
                        ...
                        InvLine.SETRANGE("Purch. Order for Extra Charge", VendorPurchaseInvoice."No.");
                        InvLine.SETRANGE("Extra Charge Code", VendorBuffer."Extra Charge Code");
        ...
        REPEAT
            ...
            IF VendorPurchaseInvoice."No." <> '' THEN
                PurchaseHeader."Extr chrg created for Ord. No." := VendorPurchaseInvoice."No.";
            ...
            REPEAT
                ...
                PurchaseLine."Extra Charge Code" := ExtraCharge.Code;
                IF VendorPurchaseInvoice."No." <> '' THEN
                    PurchaseLine."Purch. Order for Extra Charge" := VendorPurchaseInvoice."No.";
    ...
    var
        FromPstdDocExtraCharge: Record "Posted Document Extra Charges";
        ToDocExtraCharge: Record "Document Extra Charge";
    begin
        ToDocExtraCharge.SetRange("Table ID", DATABASE::"Purchase Header");
        ToDocExtraCharge.SetRange("Document Type", ToPurchHeader."Document Type");
    ...
    var
        PostedDocExtraCharge: Record "Posted Document Extra Charges";
        DocExtraCharge: Record "Document Extra Charge";
    begin
    ...
    procedure UpdateExtraChargeSummary(OrderNo: Code[20]; EntryExtraCharge: Record "Value Entry Extra Charge"; VEPostingDate: Date)
    var
    ...
    begin
        ...
        if not ExtraChargeSummary.Get(OrderNo, EntryExtraCharge."Extra Charge Code") then begin
            ...
            DocExtraCharge.Get(38, 1, OrderNo, 0, EntryExtraCharge."Extra Charge Code");
    ...
    begin
        //<<ENEC1.00
        IF (PurchLine."Purch. Order for Extra Charge" = '') OR (PurchLine."Extra Charge Code" = '') THEN
            EXIT;

        IF ExtraChargeSummary.GET(PurchLine."Purch. Order for Extra Charge", PurchLine."Extra Charge Code") THEN BEGIN
            ...
            PstdDocExtraCharge.SETFILTER("Document No.", '%1', PurchLine."Purch. Order for Extra Charge" + '*');
            PstdDocExtraCharge.SETRANGE("Extra Charge Code", PurchLine."Extra Charge Code");
            IF PstdDocExtraCharge.FINDSET THEN
                REPEAT
                    PstdDocExtraCharge.Status := PstdDocExtraCharge.Status::Closed;
                    PstdDocExtraCharge."EC Invoice No." := PurchInvLine."Document No.";
            ...
            IF PurchLine."Purch. Order for Extra Charge" = PurchLine."Document No." THEN BEGIN
                PstdDocExtraCharge.RESET;
                ...
                PstdDocExtraCharge.SETFILTER("Extra Charge Code", '<>%1', PurchLine."Extra Charge Code");
                IF PstdDocExtraCharge.FINDSET THEN
                    REPEAT
                        PstdDocExtraCharge.Status := PstdDocExtraCharge.Status::Open;
                        PstdDocExtraCharge.MODIFY;
            ...
            IF PstdDocExtraCharge.FINDSET THEN
                REPEAT
                    PurchInvLine2.RESET;
                    PurchInvLine2.SETRANGE("Extra Charge Code", PstdDocExtraCharge."Extra Charge Code");
                    PurchInvLine2.SETFILTER("Purch. Order for Extra Charge", '%1', PurchLine."Purch. Order for Extra Charge" + '*');
    ...
    procedure SetExtraChargeBuffer(VAR ExtraChargeBuffer: Record "Extra Charge Posting Buffer"; Qty: Decimal; OrderNo: Code[20])
    begin
        //<<ENEC1.00
