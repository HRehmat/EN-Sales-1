report 14228900 "Sales Guide Order Entry"
{
            ...
            begin
                ...
                if not LoadAll then
                    ...
                    if SuppChnUsrGrpFilter <> '' then // TMS1.01
                        ...
                        ILE.SetFilter("Supply Chain Group Code", SuppChnUsrGrpFilter) //TMS1.01
                    else
                        //ILE.SETFILTER("Supply Chain Group Code", UserSetup.GetUserSalesTeam); // TMS1.01
                        ILE.SetFilter("Supply Chain Group Code", SalesGuideOrderEntry.GetSupplyChainGroup); // TMS1.01

                if ILE.FindSet(false, false) then
                    repeat
                        CreateSalesGuideOrderEntry(ILE."Item No.", ILE."Variant Code", ILE."Country/Region of Origin Code");
                    until ILE.Next = 0;

                PurchLine.Reset;
                PurchLine.SetCurrentKey("Document Type", Type, "Drop Shipment", "Country/Region of Origin Code", "Variant Code",
                    ...
                    repeat
                        if not LoadAll then begin
                            if SalesTeamFilter <> '' then begin
                                ...
                                Item3.SetFilter("Supply Chain Group Code", SuppChnUsrGrpFilter);// TMS1.01
                                if Item3.FindFirst then
                                    CreateSalesGuideOrderEntry(PurchLine."No.", PurchLine."Variant Code", PurchLine."Country/Region of Origin Code");
                            end else begin
                                Item3.Reset;
                                Item3.SetRange("No.", PurchLine."No.");
                                // Item3.SETFILTER("Supply Chain Group Code",UserSetup.GetUserSalesTeam); // TMS1.01/
                                Item3.SetFilter("Supply Chain Group Code", SalesGuideOrderEntry.GetSupplyChainGroup);// TMS1.01
                                if Item3.FindFirst then
                                    CreateSalesGuideOrderEntry(PurchLine."No.", PurchLine."Variant Code", PurchLine."Country/Region of Origin Code");
                            end;
                        end else
    ...
    var
        SalesGuideOrderEntry: Record "Sales Guide Order Entry";
        LoadAll: Boolean;
        OrderNo: Code[20];
        Reload: Boolean;
        ...
        SalesGuideOrderEntry2: Record "Sales Guide Order Entry";
        LastCountry: Code[10];
        Item2: Record Item;
    ...
    begin
        if not SalesGuideOrderEntry2.Get(OrderNo, ItemNo, VariantCode) then begin
            ...
            SalesGuideOrderEntry."Supply Chain Group Code" := Item2."Supply Chain Group Code";
