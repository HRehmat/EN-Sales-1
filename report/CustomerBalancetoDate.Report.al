report 14228880 "Customer - Balance to Date Ext"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            trigger OnPreDataItem()
            begin
                //CurrReport.NewPagePerRecord := PrintOnePrPage;TBR
            end;
        }
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(PrintAmountInLCY; PrintAmountInLCY)
                    {
                        ...
                        ToolTip = 'Specifies if amounts in the report are displayed in $. If you leave the check box blank, amounts are shown in foreign currencies.';
                    }
    ...
    var
        ...
        AllamtsareinLCYCaptionLbl: Label 'All amounts are in $.';
    ...
    procedure SetCustDate(pCust: Code[20]; pMaxDate: Date)
    begin
        //EN1.00
        if pCust <> '' then begin
            CustNoFilter := pCust;
